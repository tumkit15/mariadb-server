...
if [[ "${TRAVIS_OS_NAME}" == 'linux' ]]; then
  ...
  if [[ "${CXX}" == 'clang++' ]]; then
    ...
    do
      ...
      CMAKE_OPT+=(-DCMAKE_${lang}_CREATE_SHARED_LIBRARY="${ld} <LINK_FLAGS> <CMAKE_SHARED_LIBRARY_CREATE_${lang}_FLAGS> <SONAME_FLAG><TARGET_SONAME> -o <TARGET> <OBJECTS> <LINK_LIBRARIES>")
